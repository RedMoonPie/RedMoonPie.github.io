var certifications = {
  development: [
    {
      id: 1,
      title: "Front-end Developer",
      cover:
        "./assets/certificates/Development/diploma-frontend-developer-1.jpg",
    },
    {
      id: 2,
      title: "Git y GitHub",
      cover: "./assets/certificates/Development/diploma-git-github-1.jpg",
    },
    {
      id: 3,
      title: "Profesional de Js",
      cover:
        "./assets/certificates/Development/diploma-javascript-profesional-1.jpg",
    },
    {
      id: 4,
      title: "Programación Orientada a Objetos",
      cover: "./assets/certificates/Development/diploma-oop-1.jpg",
    },
    {
      id: 5,
      title: "Buenas practicas y entorno de Desarrollo Mac",
      cover: "./assets/certificates/Development/diploma-prework-1.jpg",
    },
    {
      id: 6,
      title: "Fundamentos de JS",
      cover:
        "./assets/certificates/Development/diploma-fundamentos-javascript-1.jpg",
    },
    {
      id: 7,
      title: "React JS",
      cover: "./assets/certificates/Development/diploma-react-ejs-1.jpg",
    },
    {
      id: 8,
      title: "Ecmascript 6+",
      cover: "./assets/certificates/Development/diploma-ecmascript-6-1.jpg",
    },
    {
      id: 9,
      title: "Basico de JS",
      cover:
        "./assets/certificates/Development/diploma-basico-javascript-1.jpg",
    },

    {
      id: 10,
      title: "Asincronismo con JS",
      cover: "./assets/certificates/Development/diploma-asincronismo-js-1.jpg",
    },
    {
      id: 11,
      title: "React Router y Redux",
      cover: "./assets/certificates/Development/diploma-react-router-redux-1.jpg",
    },
    {
      id: 12,
      title: "Prework y Buenas Practicas Windows",
      cover: "./assets/certificates/Development/diploma-prework-windows-1.jpg",
    },
    {
      id: 13,
      title: "Fundamentos NodeJs",
      cover: "./assets/certificates/Development/diploma-fundamentos-node-1.jpg",
    },
    {
      id: 14,
      title: "Fundamentos de AWS Cloud",
      cover: "./assets/certificates/Development/diploma-aws-cloud-1.jpg",
    },
  ],
  management: [
    {
      id: 1,
      title: "Scrum Foundation",
      cover: "./assets/certificates/Management/Scrum-Foundation-1.jpg",
    },
    {
      id: 2,
      title: "SCRUM",
      cover: "./assets/certificates/Management/diploma-scrum-1.jpg",
    },
    {
      id: 3,
      title: "Fundamentos de Gestión de Proyectos",
      cover: "./assets/certificates/Management/diploma-gestion-1.jpg",
    },
    {
      id: 4,
      title: "Business Model Canvas",
      cover: "./assets/certificates/Management/diploma-canvas-1.jpg",
    },
    {
      id: 5,
      title: "Técnicas de Negociación",
      cover:
        "./assets/certificates/Management/diploma-tecnicas-negociacion-1.jpg",
    },
    {
      id: 6,
      title: "Diseña tu modelo de negocio",
      cover:
        "./assets/certificates/Management/definaModelo-1.jpg",
    },
  ],
  design: [
    {
      id: 1,
      title: "Design Sprint",
      cover: "./assets/certificates/Design/diploma-design-sprint-1.jpg",
    },
    {
      id: 2,
      title: "Design Thinking",
      cover: "./assets/certificates/Design/diploma-design-thinking-1.jpg",
    },
  ],
  personal: [
    {
      id: 1,
      title: "Motivación para equipos de Trabajo",
      cover: "./assets/certificates/Personal/diploma-liderazgo-equipos-1.jpg",
    },
    {
      id: 2,
      title: "Liderazgo para equipos de trabajo",
      cover: "./assets/certificates/Personal/diploma-motivacion-1.jpg",
    },
    {
      id: 3,
      title: "English as a Second Language",
      cover: "./assets/certificates/Personal/Geos_Academy.jpeg",
    },
    {
      id: 4,
      title: "IELTS 2016",
      cover: "./assets/certificates/Personal/Ielts.jpeg",
    },
  ],
};
var projects = {
  mylist: [
    {
      id: 1,
      title: "Dijikstra Python",
      cover: "../assets/Projects/Python/dijikstra.png",
      gitlink: "https://github.com/RedMoonPie/djkstra_python",
      description:
        "Shortest route algorithm, route graphing and route marking",
      type: "python",
    },
    {
      id: 2,
      title: "Stacks, Arrays, Trees",
      cover: "../assets/Projects/Python/pilas-arboles.png",
      gitlink: "https://github.com/RedMoonPie/stacks_arrays_trees",
      description:
        "Programming of data structures for the understanding of nodes and their joint operation",
        type: "python",
    },
    {
      id: 3,
      title: "Administration CRUD Web Asp.net",
      cover: "../assets/Projects/csh/crudAsp.png",
      gitlink: "https://github.com/RedMoonPie/BookListRazor",
      description:
        "Dynamic web page administration using Asp.net, in this we used concepts related to the entity framework, Asp.net core, Docker and kubernetes",
        type: "c#",
    },
    {
      id: 4,
      title: "Data base sqlServer and Docker",
      cover: "../assets/Projects/csh/sqlServer.png",
      gitlink: "https://github.com/RedMoonPie/BookListRazor",
      description:
        "Connection and link of sqlserver database with web administration page, configuration of database in Docker container",
        type: "c#",
    },
    {
      id: 5,
      title: "Medium c# topics",
      cover: "../assets/Projects/csh/Mediumg_csh.png",
      gitlink: "https://github.com/RedMoonPie/LearningCsh",
      description:
        "Different activities related to c# such as callBacks, delegates, Regular expressions, indexers, serialization etc...",
        type: "c#",
    },
    {
      id: 6,
      title: "Advanced c# topics",
      cover: "../assets/Projects/csh/Advanced_csh.png",
      gitlink: "https://github.com/RedMoonPie/coursecsh_adv",
      description:
        "Different activities related to c# such as generic classes, interfaces, Lambda expression, generic methods, predicates etc...",
        type: "c#",
    },
    {
      id: 7,
      title: "Design Patterns",
      cover: "../assets/Projects/csh/DesignPatterns.png",
      gitlink: "https://github.com/RedMoonPie/DesignPatterns_c-",
      description:
        "Practice of different design patterns, Strategy, Observer, Factory, Abs Factory, Singleton, Decorator etc...",
        type: "c#",
    },
    {
      id: 8,
      title: "Async Practice",
      cover: "../assets/Projects/js/Asynchronous.png",
      gitlink: "https://github.com/RedMoonPie/Async_Practice",
      description:
        "Understanding how callbacks work, how can we do the same with promises and the Async / Await keywords to implement asynchronous functions, API Fetching with each of those",
        type: "js",
    },
    {
      id: 9,
      title: "Media Player",
      cover: "../assets/Projects/js/MediaPlayer.png",
      gitlink: "https://github.com/RedMoonPie/MediaPlayer",
      description:
        "Understanding js Scope, closures, this, call-apply-bind, prototype in js, prototype inheritence, Parsers, Event Loop, get-set, Proxys, Generator functions, how to cancel a fetch, intersection observer, Visibility Change, TypeScript, Interfaces. Design Patterns ",
        type: "js",
    },
    {
      id: 10,
      title: "Monster Games Web Page",
      cover: "../assets/Projects/js/MonsterGamesCart.png",
      gitlink: "https://github.com/RedMoonPie/MyvideoGameStore",
      description:
        "A webpages in which I built a shopping cart using vanilla js and the local storage of the browser, apply concepts of loops, arrow functions, data loading, dynamic creation of objects, and css Styling ",
        type: "js",
    },
    {
      id: 11,
      title: "React, React redux, React router",
      cover: "../assets/Projects/js/React_redux.png",
      gitlink: "https://github.com/RedMoonPie/React-redux-router",
      description:
        "Practice of React from transforming and html/css page into a fully react page. Connecting the application with React Router and implementing React redux for better maintainability.",
        type: "js",
    },
    {
      id: 12,
      title: "Simon with js",
      cover: "../assets/Projects/js/simon.png",
      gitlink: "https://github.com/RedMoonPie/SymonWIthjs",
      description: "In this project I learned about classes, the different variables in js, closures, spread operator, immutable data structures, functions, iterators ",
        type: "js",
    },
    {
      id: 13,
      title: "Paradox Box",
      cover: "../assets/Projects/html_css/paradoxbox.png",
      gitlink: "https://github.com/RedMoonPie/paradoxBox",
      description: " Web page inspired in fantasy applying html and css concepts,I learned how to use bootstrap and jquery as well as how to manipulate the DOM",
        type: "html",
    },
    {
      id: 14,
      title: "Blue Versatile",
      cover: "../assets/Projects/html_css/blue_versatile.png",
      gitlink: "https://github.com/RedMoonPie/blue_versatilePage",
      description:
      " Basic Html and css Web Page for a small store, applied some design concepts, loading data with js, and dynamically creating html elements.",
        type: "html",
    },
  
  ],
};
